// scripts/verify/tickerContext.mjs — Phase 2 checks for getTickerContext (roadmap M5, M6);
// loaded by scripts/verify-functions.mjs with its helpers.
...
export default async function run(ctx) {
  ...
  const { computeRSI, earningsCacheState, fetchEarnings, EARNINGS_REFETCH_FLOOR_MS } = await import(ROOT + 'getTickerContext.js');
  ...
  await t('every Finnhub call 500 → 502 CONTEXT_UNAVAILABLE, per-section errors, no-store, no key material', async () => {
  ...
  await t('every Finnhub call 401 → 401 FINNHUB_KEY_REJECTED for a BYOK key, 502 for the server key; mixed failures → 502', async () => {
  ...
  await t('only /stock/candle 403 (paid endpoint, free key) → 200, technicals null, errors { candles }, cache kept at 900; a 500 there → 60', async () => {
  ...
  await t('every section answers (empty data is not an error) → 200, errors {}, max-age=900', async () => {
    setFetch(async () => ok('[]'));
    const r = await call(byok());
  ...
  await t('timeout / network failure → "timeout" / "error"; one failed market quote is not a section error', async () => {
  ...
  await t('every Finnhub call fails but Alpha Vantage earnings arrive → 200 partial, not 502', async () => {
    const request = tokenHolder();
  ...
  await t('fundamentals.forwardPE is Finnhub forwardPE, not the trailing peTTM', async () => {
  ...
  await t('revenue pairs only with the same quarter: exact date, else closest within 3 days, never cal[0]', async () => {
  ...
  await t('computeRSI is Wilder-smoothed (StockCharts reference: 70.53 → 37.77)', async () => {
  ...
  await t('earningsCacheState: fresh before next_report_date, then a 12 h fetched_at floor', async () => {
  ...
  await t('fetchEarnings: AV daily cap → stale row served and touched (one AV call); the next request skips AV', async () => {
  ...
  await t('fetchEarnings: fresh row → served from cache, no AV call', async () => {
    process.env.ALPHA_VANTAGE_KEY = 'av';
  ...
  await t('fetchEarnings: stale row + AV data → upsert with a new fetched_at, no touch', async () => {
    process.env.ALPHA_VANTAGE_KEY = 'av';
    setFetch(route([['alphavantage', () => ok(AV_BODY)]]));
    const sb = fakeClient({ row: STALE_ROW });
    const r = await fetchEarnings('AVGO', undefined, { getClient: () => sb, now });
  ...
  await t('fetchEarnings: no Supabase client + AV 500 → { data: null, reason: "HTTP 500" }', async () => {
    process.env.ALPHA_VANTAGE_KEY = 'av';
  ...
  await t('fetchEarnings: no ALPHA_VANTAGE_KEY → no-key, nothing called', async () => {
    let asked = 0;
  ...
  await t('fetchEarnings: per-minute Note / timeout / no-data reasons; cache read and write errors are best effort', async () => {
    process.env.ALPHA_VANTAGE_KEY = 'av';
    ...
    ({ r, sb } = await run(() => ok(AV_DAILY_CAP), { row: STALE_ROW, updateError: { message: 'permission denied' } }));
  ...
  await t('token holder, AV rate-limited → 200, errors.earnings, max-age=60; AV no data (ETF) is not an error', async () => {
    setFetch(route([['alphavantage', () => ok(AV_DAILY_CAP)]]));
    const r = await call(tokenHolder());
